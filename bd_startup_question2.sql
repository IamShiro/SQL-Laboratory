...
set var = (select id_programador from programador as p 
where p.id_programador not in (select id_programador from programador_linguagem));

select p.nome_programador, 
if (p.id_programador in (select id_programador from programador_linguagem), lp.linguagem, null) as linguagem
from programador as p left join programador_linguagem as pl
on p.id_programador = pl.id_programador or p.id_programador
